            ...
            <div class="card">
                <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <small>{{ $conversation->listing->title ?? 'N/A' }}</small>
                    </div>
                ...
                <div class="card-body p-0" style="height: 500px; display: flex; flex-direction: column;">
                    <!-- Messages Area -->
                    <div id="messagesArea" class="flex-grow-1 p-3 overflow-auto" style="max-height: 400px;">
                        ...
                        <div class="mb-3 d-flex {{ $message->sender_id == auth()->id() ? 'justify-content-end' : 'justify-content-start' }}">
                            <div class="message-bubble {{ $message->sender_id == auth()->id() ? 'bg-primary text-white' : 'bg-light' }}" style="max-width: 70%; padding: 10px 15px; border-radius: 18px;">
                                ...
                                <div class="small mt-1 {{ $message->sender_id == auth()->id() ? 'text-white-50' : 'text-muted' }}">
                                    {{ \App\Helpers\DateTimeHelper::format($message->created_at, 'H:i d/m/Y') }}
                                </div>
                            </div>
                        </div>
                        @endforeach
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    messageForm.addEventListener('submit', function(e) {
        ...
        if (isSending) {
            console.log('Đang gửi tin nhắn, bỏ qua request mới');
        ...
        .catch(error => {
            ...
            alert('Có lỗi xảy ra khi gửi tin nhắn: ' + (error.message || 'Unknown error') + '. Vui lòng thử lại.');
        });
        
        // Ngăn form submit mặc định (double check)
        return false;
    });
